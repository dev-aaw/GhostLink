    ...
    fn get_macos_strategies(&self, _bin_dir: &Path, socks_port: u16) -> Vec<Strategy> {
        ...
        vec![
            Strategy {
                ...
                description: "Splits at TLS ClientHello mid-domain (Google/Discord + General, port 443) and sends the second fragment first, breaking naive DPI reassembly order.".to_string(),
                platform: Platform::MacOS,
                args: [socks_args(socks_port), vec![
                    "--filter-tcp=443".to_string(),
                    format!("--hostlist={}", l("list-google.txt")),
                    "--split-pos=1,midsld".to_string(),
                    "--disorder=tls".to_string(),
                    "--new".to_string(),
                    "--filter-tcp=443".to_string(),
            ...
            Strategy {
                ...
                description: "Splits at TLS ClientHello mid-domain (Google/Discord + General, port 443) and injects an out-of-band byte, confusing DPI engines that don't handle TCP urgent data.".to_string(),
                platform: Platform::MacOS,
                args: [socks_args(socks_port), vec![
                    "--filter-tcp=443".to_string(),
                    format!("--hostlist={}", l("list-google.txt")),
            ...
            Strategy {
                ...
                description: "Splits at TLS ClientHello mid-domain (Google/Discord + General, port 443) and fragments it into 2 TLS records, defeating DPI that only inspects the first record. Deepest, most resilient option.".to_string(),
                platform: Platform::MacOS,
                args: [socks_args(socks_port), vec![
                    "--filter-tcp=443".to_string(),
                    format!("--hostlist={}", l("list-google.txt")),
            ...
            Strategy {
                ...
                description: "Lightweight, minimal split right at the TLS SNI extension boundary (port 443) with no extra tamper. Fastest option, worth trying first on lightly-censored connections.".to_string(),
                platform: Platform::MacOS,
                args: [socks_args(socks_port), vec![
                    "--filter-tcp=443".to_string(),
                    format!("--hostlist={}", l("list-general.txt")),
            ...
            Strategy {
                id: "mac-http-hostmangle".to_string(),
                name: "macOS HTTP Host Mangle".to_string(),
                description: "Plain-HTTP (port 80) Host: header mangling — lowercases it, appends a trailing dot, and mixes the domain's case. Covers ISPs that block on a cleartext Host header match rather than TLS/SNI, which none of the other strategies address.".to_string(),
                platform: Platform::MacOS,
                args: [socks_args(socks_port), vec![
